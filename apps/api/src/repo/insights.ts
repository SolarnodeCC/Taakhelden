...
export async function weeklyInsights(
  ...
  // Elk kind is een onafhankelijke batch (eigen child_id-filter, geen gedeelde
  // state) — parallel ophalen scheelt N-1 sequentiële round trips per gezin.
  const children: ChildInsightsData[] = await Promise.all(
    childRows.map((child) => loadChildInsights(db, familyId, child, startDate, endDate, today)),
  );
