...
| `HMAC_SECRET` | **yes** (`[secrets].required`) | Dedicated key for photo/export signed URLs, deliberately separate from `JWT_SECRET`. No fallback — signing fails closed. `deploy-prod.yml` generates one when the Environment has none, so it needs no manual step. |
...
2. Optionally set: `HMAC_SECRET` (auto-generated by the deploy when absent; set it explicitly only to control the value), `APNS_KEY`, `APNS_KEY_ID`, `APNS_TEAM_ID`, `EMAIL_API_KEY`, `EMAIL_FROM`.
