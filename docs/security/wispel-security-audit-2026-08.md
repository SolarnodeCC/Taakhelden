...
**Status**: ✅ **Fixed.** Migration `0012_pin_fail_count.sql` adds `users.pin_fail_count`,
...
`idempotency_keys.fingerprint` (migration 0013). Same key **and** same fingerprint returns the cached response — a genuine retry, unchanged. Same key with a different fingerprint returns `409 IDEMPOTENCY_KEY_REUSED`, so a client bug surfaces instead of silently dropping the second mutation.
